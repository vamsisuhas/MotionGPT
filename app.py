...
def create_file_message(file_path, filename):
    """Create a file download message for Gradio 5.x chatbot"""
    abs_path = os.path.abspath(file_path)
    if os.path.exists(abs_path):
        # In Gradio, files can be added as file objects in messages
        # Format: {"path": file_path, "mime_type": mime_type, "name": filename}
        return create_bot_message({
            "path": abs_path,
            "mime_type": "application/octet-stream",
            "name": filename
        })
    else:
        return create_bot_message(f"⚠️ File not found: {filename}")

def create_download_text_message(npy_path, npy_filename):
    """Create a text message with download instructions for .npy file"""
    abs_path = os.path.abspath(npy_path)
    if os.path.exists(abs_path):
        # Create a message with file download link
        # Use Gradio's file serving - the file path will be accessible
        download_text = f"📥 **Download motion data (.npy file):**\n\nClick the download button above this message to download `{npy_filename}`"
        return create_bot_message(download_text)
    else:
        return create_bot_message(f"⚠️ Motion data file not found: {npy_filename}")
...
def bot(history, motion_uploaded, data_stored, method):
    ...
    if is_motion_generation:
        ...
        # Add .npy file as downloadable file message
        npy_msg = create_file_message(output_npy_path, joints_fname)
        history = history + [npy_msg]
        yield history, motion_uploaded, data_stored
